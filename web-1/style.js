...
const job= document.querySelector(".jobTitle")
const jobContainer= document.querySelectorAll(".jobTitleContianer")
const projects= document.querySelector(".projectsTitle")
const isometrico= document.querySelector(".isometric")
...
window.addEventListener("scroll" ,()=>{
    ...
    saluteImgContainer.style.transform= `translate(${offsetY * 0.4}px, ${offsetY * 0.5}px)`
    name.style.transform = `translate(${offsetY * 0.1}px)`
    jobContainer[0].style.backgroundPositionY = `${offsetY * 0.7}px`
    job.style.transform = `translateX(calc(200vh - ${offsetY}px))`
    projects.style.transform = `translateY(${offsetY * 0.1}px)`
